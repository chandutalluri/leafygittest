  ...
  async getProductAnalytics() {
    ...
    const lowStockProducts = await db
      .select({
        ...
        minThreshold: products.lowStockThreshold
      })
      .from(inventory)
      .innerJoin(products, eq(inventory.productId, products.id))
      .where(sql`${inventory.quantity} <= ${products.lowStockThreshold}`)
  ...
  async getSystemAnalytics() {
    ...
    const notificationStats = await db
      .select({
        ...
        pendingNotifications: count(sql`CASE WHEN ${notifications.isRead} = false THEN 1 END`),
        sentNotifications: count(sql`CASE WHEN ${notifications.isRead} = true THEN 1 END`)
      })
      .from(notifications);
