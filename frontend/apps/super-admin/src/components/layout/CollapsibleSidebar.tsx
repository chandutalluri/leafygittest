...
export function CollapsibleSidebar({ activeDomain, onDomainChange }: CollapsibleSidebarProps) {
  ...
  const handleNavigation = (href: string, categoryId: string) => {
    ...
    if (href.includes('/catalog-management') || href.includes('/operational-dashboard') || href.includes('/order-management') || href.includes('/customer-service') || href.includes('/accounting-management') || href.includes('/company-management') || href.includes('/analytics-reporting') || href.includes('/database-backup-restore')) {
