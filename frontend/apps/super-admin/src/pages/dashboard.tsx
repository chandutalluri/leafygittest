...
export default function SuperAdminDashboard() {
  ...
  const getCategoryFromName = (name: string): string => {
    ...
    if (name.includes('catalog') || name.includes('inventory') || name.includes('order'))
      return 'ecommerce';
    if (name.includes('payment') || name.includes('accounting') || name.includes('expense'))
      return 'finance';
    if (name.includes('analytics') || name.includes('reporting') || name.includes('performance'))
      return 'analytics';
    if (name.includes('content') || name.includes('image') || name.includes('label'))
      return 'content';
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'core':
        return <ServerIcon className="h-6 w-6 text-blue-600" />;
      case 'ecommerce':
        return <ShoppingCartIcon className="h-6 w-6 text-green-600" />;
      case 'finance':
        return <BanknotesIcon className="h-6 w-6 text-yellow-600" />;
      case 'analytics':
        return <ChartBarIcon className="h-6 w-6 text-purple-600" />;
      case 'content':
        return <CubeIcon className="h-6 w-6 text-indigo-600" />;
      case 'logistics':
        return <TruckIcon className="h-6 w-6 text-orange-600" />;
      case 'communication':
        return <UsersIcon className="h-6 w-6 text-pink-600" />;
      default:
  ...
  if (
    ...
    microserviceComponents[currentView as keyof typeof microserviceComponents]
  ) {
  ...
  const groupedServices = microservices.reduce(
    (acc, service) => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {services.map(service => (
                        ...
                        >
                          ...
                          <div className="mt-3 text-xs text-gray-500">Uptime: {service.uptime}</div>
                        </div>
                      ))}
                    </div>
                  </div>
