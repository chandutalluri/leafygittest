...
export default function MultiLanguageManagement() {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'languages' | 'translations' | 'settings'
  >('overview');
  ...
  const loadLanguageData = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        const activeLanguages = data.data.filter(
          (lang: LanguageItem) => lang.status === 'active'
        ).length;
        const completionAverage =
          data.data.reduce((sum: number, lang: LanguageItem) => sum + lang.completion, 0) /
          totalLanguages;
        const pendingTranslations = data.data.filter(
          (lang: LanguageItem) => lang.status === 'pending'
        ).length;
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-blue-700 text-sm">
              Manage platform translations, regional settings, and internationalization features for
              global user support.
            </p>
          </div>
        </div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd
                      className={`text-lg font-medium ${getCompletionColor(stats.completionAverage)}`}
                    >
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {languages.map(language => (
                    <tr key={language.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(language.status)}`}
                        >
                          {language.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <span
                            className={`text-sm font-medium ${getCompletionColor(language.completion)}`}
                          >
                            {language.completion}%
                          </span>
                        </div>
