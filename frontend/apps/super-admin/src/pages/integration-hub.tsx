...
export default function IntegrationHub() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'integrations' | 'webhooks' | 'logs'>(
    'overview'
  );
  ...
  const loadIntegrationData = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        const activeIntegrations = data.data.filter(
          (int: Integration) => int.status === 'active'
        ).length;
        const erroredIntegrations = data.data.filter(
          (int: Integration) => int.status === 'error'
        ).length;
        const totalRequests = data.data.reduce(
          (sum: number, int: Integration) => sum + int.requests24h,
          0
        );
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-purple-700 text-sm">
              Manage all third-party integrations, APIs, webhooks, and external service connections
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {integrations.map(integration => (
                    <tr key={integration.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(integration.status)}`}
                        >
                          {integration.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {integration.requests24h.toLocaleString()}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`text-sm font-medium ${getErrorRateColor(integration.errorRate)}`}
                        >
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-lg">
            ...
            <div className="space-y-4">
              ...
              <select
                ...
                onChange={e =>
                  setNewIntegration({ ...newIntegration, type: e.target.value as any })
                }
                className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              >
